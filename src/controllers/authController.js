...
async function handleRegisterFirstUser(req, res) {
    try {
        ...
        if (count === 0) {
            const { sponsorId, name, mobileNumber, email, password } = req.body;
            ...
            const walletPoints = new WalletPoints({
                ...
                currentMonthPoints: { leftPoints: 0, rightPoints: 0 },
                directPoints: { leftPoints: 0, rightPoints: 0 },
                supportivePoints: { leftPoints: 0, rightPoints: 0 },
            });

            await walletPoints.save();
            console.log(`✅ WalletPoints schema initialized for first user ${newUser._id}`);
